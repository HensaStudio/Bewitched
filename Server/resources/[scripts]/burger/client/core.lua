...
-- CONNECTION
-----------------------------------------------------------------------------------------------------------------------------------------
vSERVER = Tunnel.getInterface("burger")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local Blip = nil
...
CreateThread(function()
	exports["target"]:AddBoxZone("WorkBurger",Init["xyz"],0.75,0.75,{
		name = "WorkBurger",
		heading = Init["w"],
		minZ = Init["z"] - 1.0,
		maxZ = Init["z"] + 1.0
...
-- BURGER:INIT
-----------------------------------------------------------------------------------------------------------------------------------------
AddEventHandler("burger:Init",function()
	if Active then
		...
		exports["target"]:LabelText("WorkBurger","Iniciar Entregas")
		Active = false
	else
		...
		exports["target"]:LabelText("WorkBurger","Finalizar Entregas")
		Active = true
		...
		if Active and not IsPedInAnyVehicle(Ped) then
			...
			if GetEntityArchetypeName(Vehicle) == Model then
				...
				if Distance <= 10.0 then
					...
					if Distance <= 1.0 and IsControlJustPressed(1,74) and vSERVER.CheckDrugs() and vSERVER.Payment(Selected) then
						Lasted = Selected
