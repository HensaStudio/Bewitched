...
function tvRP.CreateObjects(Dict, Anim, Prop, Flag, Hands, Height, Pos1, Pos2, Pos3, Pos4, Pos5)
	...
	if DoesEntityExist(Object) then
		local ObjectNet = ObjToNet(Object)
		...
		NetworkRequestControlOfEntity(ObjectHandle)
		SetEntityAsMissionEntity(ObjectHandle, true, true)
		SetEntityAsNoLongerNeeded(ObjectHandle)
		
		if DoesEntityExist(ObjectHandle) then
			DeleteEntity(ObjectHandle)
		end
		
		TriggerServerEvent("DeleteObject", ObjectNet)
		Object = nil
	end
...
function tvRP.Destroy(Mode)
	...
	if DoesEntityExist(Object) then
		...
		if not NetworkHasControlOfEntity(ObjectHandle) then
			NetworkRequestControlOfEntity(ObjectHandle)
			local TimeOut = 0
			while not NetworkHasControlOfEntity(ObjectHandle) and TimeOut < 10 do
				Wait(0)
		...
		FreezeEntityPosition(ObjectHandle, false)
