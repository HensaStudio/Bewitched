...
vRP.Prepare("characters/BankPassword","UPDATE characters SET Password = @Password WHERE id = @Passport")
vRP.Prepare("characters/BankAccess","SELECT * FROM characters WHERE id = @Passport AND Password = @Password")
...
vRP.Prepare("characters/NewCharacter","INSERT INTO characters (License,Name,Lastname,Sex,Skin,Phone,Blood,Created) VALUES (@License,@Name,@Lastname,@Sex,@Skin,@Phone,@Blood,UNIX_TIMESTAMP() + (86400 * 3))")
...
vRP.Prepare("characters/Wanted","SELECT id, Name, Lastname, Wanted FROM characters WHERE Wanted >= 1")
vRP.Prepare("characters/UpdateWanted","UPDATE characters SET Wanted = (Wanted + @Wanted) WHERE id = @Passport")
vRP.Prepare("characters/SetWanted","UPDATE characters SET Wanted = @Wanted WHERE id = @Passport")
...
vRP.Prepare("accounts/RemGemstone","UPDATE accounts SET Gemstone = Gemstone - @Gemstone WHERE License = @License")
...
vRP.Prepare("accounts/UpgradePremium","UPDATE accounts SET Premium = COALESCE(Premium, UNIX_TIMESTAMP()) + 2592000 WHERE License = @License")
-----------------------------------------------------------------------------------------------------------------------------------------
-- PLAYERDATA
-----------------------------------------------------------------------------------------------------------------------------------------
